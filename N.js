            ...
            for (let j = 0; j < this.grid; j++){
                for (let k = 0; k < this.grid; k++){
                    if(spend < this.amount){
                        let blockIteration = new Block(k + this.x,i, k + this.y);
...
class Block {
    constructor(x, y, z){
        ...
        this.z = z;
        this.vertices = [
            {x:1+this.x, y:1+this.y, z:1+this.z, flag: false},//0
            {x:1+this.x, y:-1+this.y, z:1+this.z, flag: false},//1
            {x:-1+this.x, y:-1+this.y, z:1+this.z, flag: false},//2
            {x:-1+this.x, y:1+this.y, z:1+this.z, flag: false},//3
            {x:1+this.x, y:1+this.y, z:-1+this.z, flag: false},//4
            {x:1+this.x, y:-1+this.y, z:-1+this.z, flag: false},//5
            {x:-1+this.x, y:-1+this.y, z:-1+this.z, flag: false},//6
            {x:-1+this.x, y:1+this.y, z:-1+this.z, flag: false},//7
        ];
        this.faces = [
...
class Camera {
    ...
    updateCameraMatrix() {
        ...
        this.centerMatrix = multiplyMatrices(translate, multiplyMatrices(rotZ, rotY));
        this.cameraPosition = [this.centerMatrix[0][3], this.centerMatrix[1][3], this.centerMatrix[2][3]];
        let target = multiplyMatrices(this.centerMatrix, [[1], [0], [0], [1]]);
  
        const directionVector = [[1], [0], [0], [1]];
        const rotatedTarget = multiplyMatrices(this.centerMatrix, directionVector);
        ...
        this.viewMatrix = multiplyMatrices(this.centerMatrix, lookAtMatrix);
...
let chunkNumber = 5;
let chunkGrid = 4;
let blocksInChunk = 10;
let chunkGridSize = 8;
let chunkList = [];
let camera = new Camera(0, 0, 0, 0, 0, 3.14 / 2, canvas.width/canvas.height, 1, 2000)
...
function animate() {
    ...
    ctx.fillText("camTar", camera.cameraTarget[0] + 150,camera.cameraTarget[2]);
    console.log((camera.cameraTarget[0]+ 150) )
    ...
    chunkList.forEach((chunk) => {
        chunk.blocks.forEach((block) => {
            block.faces.forEach((face) => {
                ...
                // Draw additional debug lines for the face vertices
